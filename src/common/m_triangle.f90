...
do ii=1,h_mesh%ntri
 ...
 do j=1,3
  ...
  do j=1,ny
    if (     glist%ygrd(j) .lt. x2(2) .and. x2(2) .le. glist%ygrd(j+1)) then ! lt -> le 2022.10.31
       iy=j
       do i=1,nx
         if (glist%xgrd(i) .lt. x2(1) .and. x2(1) .le. glist%xgrd(i+1)) then ! lt -> le 2022.10.31
           ix=i ; goto 100
         end if
       end do
   end if
 ...
 write(*,*) "x2(1:2)=",x2(1:2)
 write(*,*) "in classifytri2grd in m_triangle.f90"
 stop
 100 continue
