...
!#####################################################################  2024.10.08
subroutine outsitefile(g_paramslice,islice,xdir,ydir)
use post_param
use outerinnerproduct
implicit none
type(param_slice),intent(in) :: g_paramslice
real(8),intent(in) :: xdir(3),ydir(3)
integer(4),intent(in) :: islice 
integer(4) :: i,nsite
character(50),allocatable :: sitename(:)
real(8),allocatable :: xyz(:,:),x,y
character(2) :: num
character(50) :: ofile

  !# read site info
  open(1,file=g_paramslice%sitefile(islice),status='old',action='read')
  i=0
   do 
    read(1,*,end=10)
    i=i+1
   ...
   10 nsite = i
   rewind(1)
   allocate(xyz(3,nsite),sitename(nsite))
   do i=1,nsite
    read(1,*) sitename(i),xyz(1:3,i)
   end do
  close(1)

  !# out site info
  write(num,'(i2.2)') islice
  ofile=trim(g_paramslice%outpolygonhead(1))//"_"//num//"_site.dat"
  open(1,file=ofile)
  do i=1,nsite
    x = inner(xyz(1:3,i),xdir)
    y = inner(xyz(1:3,i),ydir)
    write(1,'(a,2f15.7)') trim(sitename(i)),x,y    
  end do
  close(1)
return
end
